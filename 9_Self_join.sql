...
INNER JOIN route b
  ON a.company = b.company
  AND a.num = b.num
INNER JOIN stops x ON a.stop = x.id
INNER JOIN stops y ON b.stop = y.id
WHERE
...
SELECT DISTINCT stops_b.name, b.company, b.num
FROM route a
INNER JOIN route b
  ON a.company = b.company
  AND a.num = b.num
INNER JOIN stops AS stops_a ON a.stop = stops_a.id
INNER JOIN stops AS stops_b ON b.stop = stops_b.id
WHERE
...
SELECT DISTINCT first_train.num, first_train.company, match_stop.name, second_train.num, second_train.company
FROM
(SELECT a.num, a.company, b.stop FROM route a
...
(SELECT a.num, a.company, b.stop FROM route a
	...
	WHERE name = 'Lochend')) 
        ...
        JOIN stops AS match_stop ON (first_train.stop = match_stop.id)
        ORDER BY first_train.num, match_stop.name, second_train.num;
